...
// ─── HTML email template ────────────────────────────────────────────────────
function buildHtml(type, { recipientName, patientName, doctorName, specialty, date, timeSlot }) {
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:32px;">
              ...
              <p style="margin:0 0 24px;font-size:15px;color:#374151;">${MESSAGES[type]}</p>
              <!-- Appointment Details -->
...
// ─── Send a single email ────────────────────────────────────────────────────
export const sendEmail = async ({ type, recipientEmail, recipientName, metadata }) => {
  const subject = SUBJECTS[type] || 'HC Platform — Notification';
  const html    = buildHtml(type, { recipientName, ...metadata });
